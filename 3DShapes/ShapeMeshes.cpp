...
void ShapeMeshes::LoadHemisphereMesh() {
	const int numSegments = 50;  // Number of segments to approximate the hemisphere
	...
	for (int lat = 0; lat <= numSegments / 2; ++lat) {
		float theta = lat * (M_PI / numSegments);  // Latitude angle
		float sinTheta = sin(theta);
		float cosTheta = cos(theta);

		for (int lon = 0; lon <= numSegments; ++lon) {
			float phi = lon * (2 * M_PI / numSegments);  // Longitude angle
			...
			float u = 1.0f - (float(lon) / numSegments);
			float v = 1.0f - (float(lat) / numSegments);

			verts.push_back(x);
			verts.push_back(y);
			verts.push_back(z);
			verts.push_back(x / radius);  // Normalized normal
	...
	for (int lat = 0; lat < numSegments / 2; ++lat) {
		for (int lon = 0; lon < numSegments; ++lon) {
			int first = lat * (numSegments + 1) + lon;
	...
	// Generate vertices and indices for the flat circular base
	int baseCenterIndex = verts.size() / 8;
	...
	for (int lon = 0; lon <= numSegments; ++lon) {
		float phi = lon * (2 * M_PI / numSegments);
		...
		verts.push_back(-1.0f);
		verts.push_back(0.0f);
		verts.push_back((x / radius) * 0.5f + 0.5f);
		verts.push_back((z / radius) * 0.5f + 0.5f);

		indices.push_back(baseCenterIndex);
		indices.push_back(baseCenterIndex + lon + 1);
		indices.push_back(baseCenterIndex + lon);
	}

	// Upload data to GPU (similar to your existing setup)
	m_HemisphereMesh.nVertices = verts.size() / (g_FloatsPerVertex + g_FloatsPerNormal + g_FloatsPerUV);
	m_HemisphereMesh.nIndices = indices.size();

	glGenVertexArrays(1, &m_HemisphereMesh.vao);
	glBindVertexArray(m_HemisphereMesh.vao);

	glGenBuffers(2, m_HemisphereMesh.vbos);
	glBindBuffer(GL_ARRAY_BUFFER, m_HemisphereMesh.vbos[0]);
	glBufferData(GL_ARRAY_BUFFER, verts.size() * sizeof(GLfloat), verts.data(), GL_STATIC_DRAW);

	glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, m_HemisphereMesh.vbos[1]);
...
{
	glBindVertexArray(m_HemisphereMesh.vao);
	glDrawElements(GL_TRIANGLES, m_HemisphereMesh.nIndices, GL_UNSIGNED_INT, 0);
	glBindVertexArray(0);
}
